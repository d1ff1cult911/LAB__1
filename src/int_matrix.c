...
static void int_multiply(const void* a, const void* b, void* result, int rows_a, int cols_a, int cols_b) {
    const int* a_data = a;
    const int* b_data = b;
    int* res = result;
...
static void int_add_linear_combination(void* matrix, int base_row, const void* coefficient, int rows, int cols) {
    int* data = (int*)matrix;
    int coeff = *(const int*)coefficient;
...
MatrixOperations int_matrix_ops = {
    ...
    .fill = int_fill,
    .add_linear_combination = int_add_linear_combination,
};
